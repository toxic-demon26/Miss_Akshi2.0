...
from Miss_Akshi import OWNER_ID
...
async def lego(event):
 ...
 try:
    ...
    img = Image.open('./Miss_Akshi/resources/1e0e5d446028d6e77385b4dd5d6e2254.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./Miss_Akshi/resources/Chopsic.otf", 100)
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./Miss_Akshi/resources/7ff02927132724dfc95db65bce4df3ce.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "white"
    shadowcolor = "blue"
    font = ImageFont.truetype("./Miss_Akshi/resources/Maghrib.ttf", 200)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
